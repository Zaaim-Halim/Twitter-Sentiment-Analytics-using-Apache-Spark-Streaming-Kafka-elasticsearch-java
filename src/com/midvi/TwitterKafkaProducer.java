...
public class TwitterKafkaProducer implements Runnable {

	final Logger logger = Logger.getLogger(TwitterKafkaProducer.class.getName());

	private Client client;
	private KafkaProducer<String, String> producer;
	private BlockingQueue<String> msgQueue = new LinkedBlockingQueue<>(30);
	private List<String> trackTerms = Lists.newArrayList("covid-19 vaccination", "coronavirus vaccination",
			"pandemic vaccine");
	...
	public Client createTwitterClient(BlockingQueue<String> msgQueue) {
		...
		/** Creating a client */
		ClientBuilder builder = new ClientBuilder().connectionTimeout(600000000).name("Hosebird-Client")
				.hosts(hosebirdHosts).authentication(hosebirdAuth).endpoint(hbEndpoint)
			...
			if (msg != null) {
				...
				producer.send(new ProducerRecord<String, String>(KafkaConfig.TOPIC, null, msg), new Callback() {
					...
					public void onCompletion(RecordMetadata recordMetadata, Exception e) {
						if (e != null) {
							logger.info("Some error OR something bad happened : " + e.toString());
